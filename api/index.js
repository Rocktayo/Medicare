...
var DATA_FILE = isVercel ? "/tmp/local_db_backup.json" : path.join(process.cwd(), "local_db_backup.json");
var defaultUsers = [
  {
    id: "USR-003",
    patientId: "ADM-0001",
...
async function connectToMongoDB() {
  ...
  try {
    dbClient = new MongoClient(MONGODB_URI, { connectTimeoutMS: 4e3 });
...
app.get("/api/health", async (req, res) => {
  ...
  res.json({
    ...
    database: isMongoConnected ? "MongoDB Cloud/Cluster Database" : process.env.MONGODB_URI ? "MongoDB Connecting..." : "MongoDB Local Store Engine",
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const targetRole = role === "Admin" ? "Admin" : "Patient";
    ...
    if (isMongoConnected && db) {
      ...
      const patientId = targetRole === "Admin" ? `ADM-${Math.floor(1e3 + Math.random() * 9e3)}` : `MED-${Math.floor(1e5 + Math.random() * 9e5)}`;
      const newUserDoc = {
        patientId,
        fullName,
        email: lowerEmail,
        ...
        role: targetRole,
        age: targetRole === "Patient" ? Number(age) || 30 : void 0,
        gender: targetRole === "Patient" ? gender || "Not Specified" : void 0,
        bloodGroup: targetRole === "Patient" ? bloodGroup || "O+" : void 0,
        phone: phone || "",
...
app.post("/api/appointments", async (req, res) => {
  try {
    ...
    if (isMongoConnected && db) {
      ...
      localData.appointments.unshift(newApt);
      saveLocalData();
    }
...
app.patch("/api/appointments/:id", async (req, res) => {
  try {
    ...
    if (isMongoConnected && db) {
      ...
      if (idx !== -1) {
        localData.appointments[idx].status = status;
        saveLocalData();
      }
    }
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`\u{1F680} MediCare HMS Server running at http://0.0.0.0:${PORT}`);
  });
}
